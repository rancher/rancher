...
Rancher updates in real time by monitoring Docker events on all hosts. So when a container is started, stopped, or destroyed outside of Rancher (for example by executing `docker stop sad_einstein` directly on a host), Rancher will detect that change and update its states accordingly. Note that one current limitation is that we wait until containers are started (not created) to import them to Rancher. So, running `docker create ubuntu` will not cause the container to appear in the Rancher UI, but running `docker start ubuntu` or `docker run ubuntu` will.
...
```
To read more about the Rancher managed network and cross-host networking, see the **[Concepts]({{site.baseurl}}/docs/concepts/)** article.

### Importing Existing Containers

Rancher also supports importing existing container upon host registration. When you register a host, any containers currentrly on the host will be detected and imported into Rancher.
