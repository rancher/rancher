...
With Rancher, you can add multiple services in a stack to make an application. With this guide, we'll assume you've already created a [stack]({{site.baseurl}}/docs/rancher-ui/applications/stacks/), set up your [hosts]({{site.baseurl}}/docs/rancher-ui/infrastructure/hosts/), and are ready to build your application. 
...
Just like adding individual [containers]({{site.baseurl}}/docs/rancher-ui/infrastructure/containers/), any options that `docker run` supports, Rancher also supports! Port mapping and service links are shown on the main page, but other options are within the **Advanced Options** section. 
...
At this point, it would make sense to load balance our Wordpress service. Let's move on to how to [add a balancer service]({{site.baseurl}}/docs/rancher-ui/applications/stacks/adding-balancers/) into our stack.
