...
func NewEC2Client() (*EC2Client, error) {
	...
	credential := credentials.NewStaticCredentials(configuration.GetAWSAccessKeyID(), configuration.GetAWSSecretAccessKey(), "")
	sess, err := session.NewSession(&aws.Config{
		Credentials: credential,
		Region:      aws.String(configuration.GetAWSRegion())},
	)
	if err != nil {
		return nil, err
	}
...
func (e *EC2Client) CreateNodes(nodeNameBase string, publicIp bool, numOfInstancs int64) (func() error, error) {
	configuration := config.GetInstance()
	sshName := getSSHKeyName(configuration.GetAWSSSHKeyName())

	runInstancesInput := &ec2.RunInstancesInput{
		ImageId:      aws.String(configuration.GetAWSAMI()),
		InstanceType: aws.String(configuration.GetAWSInstanceType()),
		...
		BlockDeviceMappings: []*ec2.BlockDeviceMapping{
			{
				...
				Ebs: &ec2.EbsBlockDevice{
					VolumeSize: aws.Int64(int64(configuration.GetAWSVolumeSize())),
				},
			},
		},
		IamInstanceProfile: &ec2.IamInstanceProfileSpecification{
			Name: aws.String(configuration.GetAWSIAMProfile()),
		},
		Placement: &ec2.Placement{
			AvailabilityZone: aws.String(configuration.GetAWSRegionAZ()),
		},
		NetworkInterfaces: []*ec2.InstanceNetworkInterfaceSpecification{
			{
				DeviceIndex:              aws.Int64(0),
				AssociatePublicIpAddress: aws.Bool(publicIp),
				Groups:                   aws.StringSlice([]string{configuration.GetAWSSecurityGroup()}),
			},
		},
		TagSpecifications: []*ec2.TagSpecification{
			{
				...
				Tags: []*ec2.Tag{
					...
					{
						Key:   aws.String("CICD"),
						Value: aws.String(configuration.GetAWSCICDInstanceTag()),
					},
				},
			},
		},
	...
	sshKey, err := getSSHKey(configuration.GetAWSSSHKeyName())
	if err != nil {
		return insanceCleanup, err
	}
	...
	for _, readyInstance := range readyInstances {
		ec2Node := &EC2Node{
			...
			SSHUser:          configuration.GetAWSUser(),
			SSHKey:           sshKey,
		}
