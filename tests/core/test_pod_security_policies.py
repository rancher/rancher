...
def setup_cluster_with_pspt(client, request, template="pspt1"):
    """
    Sets the 'local' cluster to have the pspt given by template, if it exists, returns the pspt and cleans up. If it does, it creates a copy and applies
    it. If it does not, it creates a generic one
    :param client:
    :param request:
    :param template:
    :return pspt:
...
def test_pod_security_policy_template_deletion(admin_mc, request, remove_resource):
    ...
    #  create a project in order to establish bindings (there might be a better way to create binding)
    #  shameless copy from test_rbac.py
    def create_project():
        try:
            p = api_client.create_project(name="test-unrestrict-proj", clusterId="local")
            api_client.wait_success(p)
            p.setpodsecuritypolicytemplate(podSecurityPolicyTemplateId=pspt_proj.id)
    ...
    # When this returns, a project has successfully been created in the local cluster
    assert proj.state == 'active'
    assert proj.podSecurityPolicyTemplateId == pspt_proj.id

    # delete the project so there is nothing preventing the binding from being removed
