...
def wait_to_ensure_user_in_crb_subject(api_instance, name,
                                       userId, timeout=45):
    found = False
    interval = 0.5
    start = time.time()
    while not found:
        time.sleep(interval)
        interval *= 2
        crb = api_instance.read_cluster_role_binding(name)
        for i in range(0, len(crb.subjects)):
            if crb.subjects[i].name == userId:
                found = True
        if time.time() - start > timeout:
            raise AssertionError(
                "Timed out waiting for user to get added to crb")


def wait_to_ensure_user_removed_from_crb_subject(api_instance, name,
                                                 userId, timeout=45):
    removed = False
    interval = 0.5
    start = time.time()
    while not removed:
        ...
        if time.time() - start > timeout:
            raise AssertionError(
                "Timed out waiting for user to get removed from crb")
