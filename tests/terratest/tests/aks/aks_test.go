...
func TestAKSDownStreamCluster(t *testing.T) {
	...
	config1 := functions.SetConfigTF(config.Aks, config.AKSK8sVersion1235, config.Nodes3_Etcd1_Cp1_Wkr1)
	...
	assert.Equal(t, terraform.Output(t, terraformOptions, "expected_kubernetes_version_1235"), cluster.Version.GitVersion)
