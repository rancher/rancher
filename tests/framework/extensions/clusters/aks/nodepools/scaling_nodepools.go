...
func ScalingAKSNodePools(client *rancher.Client, oldCluster *management.Cluster, displayName, cloudCredentialID string) (*management.Cluster, error) {
	logrus.Infof("======================================")
	logrus.Infof("TEST CASE: Scaling AKS Node Pools")
	logrus.Infof("======================================")

	nodePool, err := np.CreateNodePool()
	if err != nil {
		return nil, err
	}
