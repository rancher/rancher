...
const (
	...
	baseDOClusterName               = "v1autodocluster"
	baseCustomClusterName           = "v1autocustomcluster"
...
func CreateEC2Instances(s *ProvisioningTestSuite) {
	...
	s.allNodes = append(s.allNodes, s.nodes...)
}
...
type ProvisioningTestSuite struct {
	...
	digitalOceanCloudCredential       map[string]*cloudcredentials.CloudCredential
	digitalOceanCloudCredentialConfig map[string]*v3.CloudCredential
...
func (s *ProvisioningTestSuite) SetupSuite() {
	...
	testCase := ""
	for _, comandLineString := range testifyArg {
		if strings.Contains(comandLineString, "^") {
			testCase = comandLineString
		}
	}

	if strings.Contains(testCase, "DigitalOcean") || testCase == "" {
		s.digitalOceanCloudCredential = make(map[string]*cloudcredentials.CloudCredential)
		s.digitalOceanCloudCredentialConfig = make(map[string]*v3.CloudCredential)
		for userName, bearerToken := range clients.BearerTokensList() {
			...
			s.digitalOceanCloudCredential[userName] = cloudCredential
			s.digitalOceanCloudCredentialConfig[userName] = returnedDoCloudCred
		}
	}
	...
	for s.bearerName, s.bearerToken = range clients.BearerTokensList() {
		...
		for _, tt := range tests {
			...
			s.Run(name, func() {
				...
				ProvisionCluster(s, s.digitalOceanCloudCredentialConfig[s.bearerName].ID)
...
func (s *ProvisioningTestSuite) TearDownSuite() {
	if s.rancherCleanup {
		err := cluster.ClusterCleanup(s.ec2Client, s.clusterObjs, s.clusterNames, s.allNodes)
		require.NoError(s.T(), err)
		if s.digitalOceanCloudCredential != nil {
			...
			for s.bearerName = range clients.BearerTokensList() {
				err := s.digitalOceanCloudCredential[s.bearerName].DeleteCloudCredential(s.digitalOceanCloudCredentialConfig[s.bearerName])
				require.NoError(s.T(), err)
			}
		}
	}
