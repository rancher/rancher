...
func (u *UpgradeTestSuite) TestPauseUpdateFleetRepo() {
	...
	gitRepo.Spec.Paused = false
	repoObject, err = u.client.Steve.SteveType(extensionsfleet.FleetGitRepoResourceType).Update(repoObject, gitRepo)
	require.NoError(u.T(), err)
	...
	gitRepoUnpause, err := u.client.Steve.SteveType(extensionsfleet.FleetGitRepoResourceType).ByID(repoObject.ID)
	require.NoError(u.T(), err)

	u.T().Log("Checking Git Commit")
	newGitStatus := &v1alpha1.GitRepoStatus{}
	err = steveV1.ConvertToK8sType(gitRepoUnpause.Status, newGitStatus)
	require.NoError(u.T(), err)
	require.Equal(u.T(), gitRepo.Status.Commit, newGitStatus.Commit)
}
