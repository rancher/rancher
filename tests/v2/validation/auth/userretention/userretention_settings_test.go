...
// Helper function to test positive input values
func (ur *UserRetentionSettingsTestSuite) testPositiveInputValues(settingName string, tests []struct {
	value       string
	description string
}) {
	log.Infof("Updating %s settings with positive values:", settingName)
	for _, inputValue := range tests {
		ur.T().Run(inputValue.value, func(*testing.T) {
			...
			log.Infof("%s setting is updated to %s ; %s", settingName, inputValue.value, inputValue.description)
		})
	}
}

// Helper function to test negative input values
func (ur *UserRetentionSettingsTestSuite) testNegativeInputValues(settingName string, tests []struct {
	value       string
	description string
}) {
	log.Infof("Updating %s settings with negative values:", settingName)
	for _, inputValue := range tests {
		ur.T().Run(inputValue.value, func(*testing.T) {
			err := updateUserRetentionSettings(ur.client, settingName, inputValue.value)
			...
			log.Infof("Failed to update %s settings to %s; %s", settingName, inputValue.value, inputValue.description)
		})
	}
}
...
// Helper function to validate settings were not updated
