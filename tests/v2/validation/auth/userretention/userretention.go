...
func setupUserRetentionSettings(client *rancher.Client, disableAfterValue string, deleteAfterValue string, userRetentionCronValue string, dryRunValue string) error {
	logrus.Info("Setting up userretention settings TO : ")
	...
	err := updateUserRetentionSettings(client, disableInactiveUserAfter, defaultSettingValue, disableAfterValue)
	if err != nil {
		return fmt.Errorf("failed to update disable-inactive-user-after setting: %w", err)
	}
	err = updateUserRetentionSettings(client, deleteInactiveUserAfter, defaultSettingValue, deleteAfterValue)
	if err != nil {
		return fmt.Errorf("failed to update delete-inactive-user-after setting: %w", err)
	}

	err = updateUserRetentionSettings(client, userRetentionCron, defaultSettingValue, userRetentionCronValue)
	if err != nil {
		return fmt.Errorf("failed to update user-retention-cron setting: %w", err)
	}
	err = updateUserRetentionSettings(client, userRetentionDryRun, defaultSettingValue, dryRunValue)
	if err != nil {
		return fmt.Errorf("failed to update user-retention-dry-run setting: %w", err)
	}
	logrus.Info("userretention settings setup completed")
	return nil
}

func updateUserRetentionSettings(client *rancher.Client, settingName string, settingDefaultValue string, settingValue string) error {
	...
	setting.Default = settingDefaultValue
