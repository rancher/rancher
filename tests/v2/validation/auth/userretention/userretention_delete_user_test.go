...
func (ur *URDeleteTestSuite) TestUserIsNotGetDeletedWithDryRun() {
	...
	logrus.Info("Resetting userretention dry run setting")
	updateUserRetentionSettings(ur.client, userRetentionDryRun, defaultSettingValue, "false")
}

func (ur *URDeleteTestSuite) TestStandardUserGetDeletedWithDefaultValue() {
	logrus.Info("Setting up userretention settings")
	setupUserRetentionSettings(ur.client, "400h", "", "*/1 * * * *", "false")
	logrus.Info("Update deleteInactiveUserAfter settings with Default value")
	updateUserRetentionSettings(ur.client, deleteInactiveUserAfter, "400h", "")
