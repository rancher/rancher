...
func (c *CustomClusterDeleteTestSuite) TestDeletingRKE1CustomCluster() {
	rolesPerNode := []string{}
		...
		for _, nodeProviderName := range c.nodeProviders {
			...
			for _, kubeVersion := range c.kubernetesVersions {
				...
				for _, cni := range c.cnis {
					name += " cni: " + cni
					c.Run(name, func() {
						cluster, err := TestProvisioningRKE1CustomCluster(c.T(), client, externalNodeProvider, nodesAndRoles, tt.psact, kubeVersion, cni, c.advancedOptions)
						require.NoError(c.T(), err)

						TestDeletingRKE1Cluster(c.T(), client, cluster)
					})
				}
			}
		}
