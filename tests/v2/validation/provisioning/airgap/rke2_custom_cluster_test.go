...
func (a *AirGapRKE2CustomClusterTestSuite) TestProvisioningAirGapUpgradeRKE2CustomCluster() {
	...
	for _, tt := range tests {
		...
		a.Run(tt.name, func() {
			...
			err = v1.ConvertToK8sType(clusterObject, &updatedClusterObject)
			require.NoError(a.T(), err)
			...
			upgradedCluster, err := clusters.UpdateK3SRKE2Cluster(a.client, clusterObject, updatedClusterObject)
			require.NoError(a.T(), err)
