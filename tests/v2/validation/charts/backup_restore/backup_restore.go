...
func verifyRancherResources(client *rancher.Client, curUserList []*management.User, curProjList []*management.Project, curRoleList []*management.RoleTemplate) (error, error, error) {
	var userErr error
	var projErr error
	var roleErr error

	logrus.Info("Verifying user resources...")
	for i := range curUserList {
		user, err := users.GetUserIDByName(client, curUserList[i].Name)
		if err != nil {
			userErr = err
		}
		if user == "" {
	...
	for i := range curProjList {
		_, err := client.Management.Project.ByID(curProjList[i].ID)
		if err != nil {
			projErr = err
		}
	}
	...
	for i := range curRoleList {
		_, err := client.Management.RoleTemplate.ByID(curRoleList[i].ID)
		if err != nil {
			roleErr = err
		}
	}

	return userErr, projErr, roleErr
}
