...
func (b *BackupTestSuite) TestBackupChart() {
	...
	initialBackupChart, err := charts.GetChartStatus(b.client, project.ClusterID, backupChartNamespace, backupChartName)
	require.NoError(b.T(), err)
	...
	userList, projList, roleList, err := createRancherResources(client, 2, project.ClusterID)
	require.NoError(b.T(), err)
	...
	backupPresent, err := validateAWSS3BackupExists(backupRestoreConfig.S3BucketName, backupRestoreConfig.S3FolderName, backupFileName)
	require.NoError(b.T(), err)
	assert.True(b.T(), backupPresent)

	userListPostBackup, projListPostBackup, roleListPostBackup, err := createRancherResources(client, 2, project.ClusterID)
	require.NoError(b.T(), err)
	...
	clusterList = append(clusterList, backupRestoreConfig.Rke1ClusterName)
	clusterList = append(clusterList, backupRestoreConfig.Rke2ClusterName)

	for i := 0; i < len(clusterList); i++ {
		clusterID, err := clusters.GetClusterIDByName(client, clusterList[i])
		require.NoError(b.T(), err)
	...
	err = deleteAWSBackup(backupRestoreConfig.S3BucketName, backupRestoreConfig.S3FolderName, backupFileName)
	require.NoError(b.T(), err)
	backupDeleted, err := validateAWSS3BackupExists(backupRestoreConfig.S3BucketName, backupRestoreConfig.S3FolderName, backupFileName)
