...
func (c *CisBenchmarkTestSuite) SetupSuite() {
	...
	projectConfig := &management.Project{
		ClusterID: clusterID,
		Name:      CISBenchmarkProjectName,
	}
	createdProject, err := client.Management.Project.Create(projectConfig)
	require.NoError(c.T(), err)
	require.Equal(c.T(), createdProject.Name, CISBenchmarkProjectName)
...
func (c *CisBenchmarkTestSuite) TestUpgradeCisbenchmarkChart() {
	...
	cisbenchmarkChartPreUpgrade, err := extencharts.GetChartStatus(client, c.project.ClusterID, charts.CISBenchmarkNamespace, charts.CISBenchmarkName)
	require.NoError(c.T(), err)
	...
	c.chartInstallOptions.Version, err = client.Catalog.GetLatestChartVersion(charts.CISBenchmarkName, catalog.RancherChartRepo)
	require.NoError(c.T(), err)
	scanName := namegen.AppendRandomString("scan")
	...
	cisbenchmarkChartPostUpgrade, err := extencharts.GetChartStatus(client, c.project.ClusterID, charts.CISBenchmarkNamespace, charts.CISBenchmarkName)
	require.NoError(c.T(), err)
...
func checkScanReport(steveClient *v1.Client, scanName string) error {
	...
	cisReportSpec := &ClusterScanReportSpec{}
...
func runScan(client *rancher.Client, scanName, profileName, clusterID string) error {
	...
	yamlInput := &management.ImportClusterYamlInput{
		DefaultNamespace: charts.CISBenchmarkNamespace,
	...
	err = wait.Poll(defaults.FiveHundredMillisecondTimeout, defaults.FiveMinuteTimeout, func() (done bool, err error) {
		scanList, err := steveClient.SteveType(ClusterScanResourceType).List(nil)
		if err != nil {
			return false, err
		}
