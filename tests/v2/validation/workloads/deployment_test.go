...
func (d *DeploymentTestSuite) TestDeploymentSideKick() {
	...
	createdDeployment, err := deployment.CreateDeploymentWithConfigmap(d.client, d.cluster.ID, namespace.Name, 1, "", "", false, false)
	require.NoError(d.T(), err)

	log.Info("Waiting for all pods to be running")
	err = pods.WatchAndWaitPodContainerRunning(d.client, d.cluster.ID, namespace.Name, createdDeployment, nginxImageName, 1)
	require.NoError(d.T(), err)
	...
	log.Info("Waiting for all pods to be running")
	err = pods.WatchAndWaitPodContainerRunning(d.client, d.cluster.ID, namespace.Name, updatedDeployment, nginxImageName, 1)
	require.NoError(d.T(), err)
}
...
func (d *DeploymentTestSuite) TestDeploymentUpgrade() {
	...
	log.Info("Creating new deployment")
	upgradeDeployment, err := deployment.CreateDeploymentWithConfigmap(d.client, d.cluster.ID, namespace.Name, 2, "", "", false, false)
	require.NoError(d.T(), err)
...
func (d *DeploymentTestSuite) TestDeploymentPodScaleUp() {
	...
	log.Info("Creating new deployment")
	scaleUpDeployment, err := deployment.CreateDeploymentWithConfigmap(d.client, d.cluster.ID, namespace.Name, 1, "", "", false, false)
	require.NoError(d.T(), err)
...
func (d *DeploymentTestSuite) TestDeploymentPodScaleDown() {
	...
	log.Info("Creating new deployment")
	scaleDownDeployment, err := deployment.CreateDeploymentWithConfigmap(d.client, d.cluster.ID, namespace.Name, 3, "", "", false, false)
	require.NoError(d.T(), err)
...
func (d *DeploymentTestSuite) TestDeploymentPauseOrchestration() {
	...
	log.Info("Creating new deployment")
	pauseDeployment, err := deployment.CreateDeploymentWithConfigmap(d.client, d.cluster.ID, namespace.Name, 2, "", "", false, false)
	require.NoError(d.T(), err)
	...
	log.Info("Waiting for all pods to be running")
	err = pods.WatchAndWaitPodContainerRunning(d.client, d.cluster.ID, namespace.Name, pauseDeployment, nginxImageName, int(replicas))
	require.NoError(d.T(), err)
