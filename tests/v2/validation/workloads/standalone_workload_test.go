...
func (w *WorkloadTestSuite) TestWorkloadDeployment() {
	...
	_, err := WorkloadDeploymentTest(w.client, w.cluster.ID)
	require.NoError(w.T(), err)
}
...
func (w *WorkloadTestSuite) TestWorkloadSideKick() {
	...
	_, err := WorkloadSideKickTest(w.client, w.cluster.ID)
	require.NoError(w.T(), err)
}
...
func (w *WorkloadTestSuite) TestWorkloadDaemonSet() {
	...
	_, err := WorkloadDaemonSetTest(w.client, w.cluster.ID)
	require.NoError(w.T(), err)
}
...
func (w *WorkloadTestSuite) TestWorkloadCronjob() {
	...
	_, err := WorkloadCronjobTest(w.client, w.cluster.ID)
	require.NoError(w.T(), err)
}
...
func (w *WorkloadTestSuite) TestWorkloadStatefulset() {
	...
	_, err := WorkloadStatefulsetTest(w.client, w.cluster.ID)
	require.NoError(w.T(), err)
}
...
func (w *WorkloadTestSuite) TestWorkloadUpgrade() {
	...
	_, err := WorkloadUpgradeTest(w.client, w.cluster.ID)
	require.NoError(w.T(), err)
}
...
func (w *WorkloadTestSuite) TestWorkloadPodScaleUp() {
	...
	_, err := WorkloadPodScaleUpTest(w.client, w.cluster.ID)
	require.NoError(w.T(), err)
}
...
func (w *WorkloadTestSuite) TestWorkloadPodScaleDown() {
	...
	_, err := WorkloadPodScaleDownTest(w.client, w.cluster.ID)
	require.NoError(w.T(), err)
}
...
func (w *WorkloadTestSuite) TestWorkloadPauseOrchestration() {
	...
	_, err := WorkloadPauseOrchestrationTest(w.client, w.cluster.ID)
	require.NoError(w.T(), err)
}
