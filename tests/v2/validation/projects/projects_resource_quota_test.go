...
import (
	...
	quotas "github.com/rancher/shepherd/extensions/kubeapi/resourcequotas"
...
func (prq *ProjectsResourceQuotaTestSuite) TestOverrideQuotaInNamespace() {
	...
	_, err = deployment.UpdateDeployment(standardUserClient, prq.cluster.ID, createdDeployment, currentDeployment)
	require.NoError(prq.T(), err)
...
func (prq *ProjectsResourceQuotaTestSuite) TestMoveNamespaceFromNoQuotaToQuotaProject() {
	...
	log.Info("Verify that the resource quota validation for the namespace is successful.")
	err = checkNamespaceResourceQuotaValidationStatus(standardUserClient, prq.cluster.ID, updatedNamespace.Name, namespacePodLimit, true, "")
	require.NoError(prq.T(), err)
	...
	require.NoError(prq.T(), err)
	currentDeployment.Spec.Replicas = &replicas
	updatedDeployment, err := deployment.UpdateDeployment(standardUserClient, prq.cluster.ID, createdDeployment, currentDeployment)
	require.NoError(prq.T(), err)
