...
func (pr *ProjectsTestSuite) TestProjectWithResourceQuotaAndContainerDefaultResourceLimit() {
	...
	createdDeployment, err := deployment.CreateDeployment(standardUserClient, pr.cluster.ID, currentNamespace.Name, 2, "", "", false, false)
	require.NoError(pr.T(), err, "Failed to create deployment in the namespace")
	err = charts.WatchAndWaitDeployments(standardUserClient, pr.cluster.ID, currentNamespace.Name, metav1.ListOptions{
