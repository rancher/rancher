...
func (p *PortTestSuite) TestClusterIP() {
	...
	err = services.VerifyClusterIP(p.client, p.cluster.ID, steveClient, serviceResp.ID, fmt.Sprintf("%d/name.html", port), daemonsetName)
	require.NoError(p.T(), err)
}
...
func (p *PortTestSuite) TestClusterIPScaleAndUpgrade() {
	...
	err = services.VerifyClusterIP(p.client, p.cluster.ID, steveClient, serviceResp.ID, fmt.Sprintf("%d/name.html", port), deploymentName)
	require.NoError(p.T(), err)
