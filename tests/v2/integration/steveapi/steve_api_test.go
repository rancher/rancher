...
func (s *SteveAPITestSuite) SetupSuite() {
	...
	// create project
	projectName := provisioning.AppendRandomString(projectNamePrefix)
	...
	for k, _ := range namespaceMap {
		name := provisioning.AppendRandomString(k)
	...
	for user, access := range testUsers {
		username := provisioning.AppendRandomString(user)
		...
		case []rbacv1.RoleBinding:
			for _, rb := range binding {
				...
				_, err = rbac.CreateRoleBinding(s.client, s.project.ClusterID, provisioning.AppendRandomString(rb.Name), namespaceMap[rb.Namespace], rb.RoleRef.Name, subject)
			}
		}
...
func (s *SteveAPITestSuite) TestList() {
	...
	for _, test := range tests {
		s.Run(test.description, func() {
			...
			require.NoError(s.T(), err)
			jsonFilePath := path.Join(jsonDir, test.description+".json")
...
func getCurlURL(client *clientv1.Client, namespace, query string) (string, error) {
	curlURL, err := client.APIBaseClient.Ops.GetCollectionURL(stevesecrets.SecretSteveType)
	if err != nil {
		return "", err
	}
	if namespace != "" {
