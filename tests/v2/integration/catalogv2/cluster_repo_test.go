...
func (c *ClusterRepoTestSuite) TestOCIRepoChartInstallation() {
	...
	err = c.waitForChart(rv1.StatusDeployed, chartInstallAction.Charts[0].ReleaseName, 0)
	require.NoError(c.T(), err)
...
func (c *ClusterRepoTestSuite) waitForChart(status rv1.Status, name string, previousVersion int) error {
	...
	err := kwait.Poll(time.Duration(500*time.Millisecond), time.Duration(t)*time.Second, func() (done bool, err error) {
		app, err = c.catalogClient.Apps(namespace.UIPluginNamespace).Get(context.TODO(), name, metav1.GetOptions{})
