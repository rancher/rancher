...
func Start429Registry(t assert.TestingT) (*url.URL, error) {
	helmChartTar, err := os.ReadFile("../../../testdata/testingchart-0.1.0.tgz")
	...
	helmChartTar2, err := os.ReadFile("../../../testdata/testchart-1.0.0.tgz")
	assert.NoError(t, err)
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v2/testingchart/blobs/" + layerDesc.Digest.String():
			http.ServeFile(w, r, "../../../testdata/testingchart-0.1.0.tgz")
		case "/v2/testchart/blobs/" + layerDesc2.Digest.String():
			http.ServeFile(w, r, "../../../testdata/testchart-1.0.0.tgz")
...
func (c *ClusterRepoTestSuite) test4xxErrors(params ClusterRepoParams) {
	...
	err = wait.Poll(PollInterval, 5*time.Second, func() (done bool, err error) {
		clusterRepo, err := c.client.Steve.SteveType(catalog.ClusterRepoSteveResourceType).ByID(params.Name)
		require.NoError(c.T(), err)
		status := c.getStatusFromClusterRepo(clusterRepo)
		if clusterRepo.Name != params.Name {
			return false, nil
		}
