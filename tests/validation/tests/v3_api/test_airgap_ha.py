...
RKE_VERSION = os.environ.get("RKE_VERSION", "v1.2.8")
...
def setup_airgap_rancher(bastion_node, number_of_nodes=NUMBER_OF_INSTANCES):
    ...
    rke_template_node_beginning = \
        'nodes:\n'
    rke_template_node_end = \
        '  user: ubuntu\n' \
        '  role:\n'  \
        '  - controlplane\n' \
        '  - etcd\n' \
    ...
    for x in range(number_of_nodes):
        rke_template_node_beginning += '- address: ' \
    ...
    rke_template_node_beginning +=\
        ...
        'private_registries:\n' \
        '- url: {}\n' \
        '  is_default: true'.format(REGISTRY_HOSTNAME)
    # write config, and run rke up
    bastion_node.execute_command(
        "echo '{}' > config.yaml".format(rke_template_node_beginning))
    bastion_node.execute_command(
