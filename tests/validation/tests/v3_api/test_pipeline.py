...
def create_project_client(request):
    ...
    def fin():
        #project delete does not remove all ns created in test, removing manually
...
def test_run_example_pipeline():
    ...
    assert len(namespace["p_client"].list_pipeline(projectId=namespace["project"].id).data) > 0
    ...
    namespace["pipeline_run"] = namespace["p_client"].action(obj=pipeline,
                                action_name='run',
                                branch="master")
    wait_for_condition(namespace["p_client"], pipeline, check_last_run_state("Success"), timeout=600)
