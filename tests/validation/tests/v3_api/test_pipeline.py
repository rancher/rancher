...
def test_pipeline():
    ...
    pipeline_details["pipeline_run"] = pipeline_details["p_client"].action(obj=pipeline,
                                                             action_name='run',
                                                             branch="master")
    wait_for_condition(
        pipeline_details["p_client"], pipeline, check_last_run_state("Success"),
...
def create_project_client(request):
    ...
    def fin():
        ...
        client = get_cluster_client_for_token(pipeline_details["cluster"], USER_TOKEN)
        for ns_x in client.list_namespace(projectId=pipeline_details["project"].id):
            client.delete(ns_x)
