...
func NewImportedClusterPods(clients *clients.Clients, namespace, k8sVersion string, pools []ImportedNodePool, labels map[string]string, registryCACert []byte, opts ...ImportedClusterOptions) ([]*corev1.Pod, error) {
	...
	initScript := importedNodeUserData(distro, k8sVersion, token, "imported-init-0", "", true, initSpec.cp, initSpec.etcd, initSpec.worker, registryCACert, options)
	...
	for i, spec := range joinSpecs {
		...
		script := importedNodeUserData(distro, k8sVersion, token, nodeName, serverURL, false, spec.cp, spec.etcd, spec.worker, registryCACert, options)
...
func importedNodeUserData(distro, k8sVersion, token, nodeName, serverURL string, isInit, cp, etcd, worker bool, registryCACert []byte, options ImportedClusterOptions) string {
	isServer := etcd || cp
