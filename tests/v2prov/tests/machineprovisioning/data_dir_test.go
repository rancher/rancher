...
func Test_Operation_SetC_MP_DataDirectories(t *testing.T) {
	...
	for _, machine := range machines.Items {
		...
		if err != nil {
			t.Fatalf("failed to get %s %s/%s to print error: %v", machine.Spec.InfrastructureRef.GroupVersionKind().String(), machine.Spec.InfrastructureRef.Namespace, machine.Spec.InfrastructureRef.Name, newErr)
		}
