...
// lifecycle through beacons and plan secrets.
func RunEncryptionKeyRotationOperationTest(t *testing.T, clients *clients.Clients, namespace string, clusterRef corev1.ObjectReference) *opv1alpha1.EncryptionKeyRotation {
	t.Helper()
	...
	op, err := clients.Operation.EncryptionKeyRotation().Create(op)
	if err != nil {
		t.Fatal(err)
	}
