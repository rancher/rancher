...
func RunSnapshotRestoreTest(t *testing.T, clients *clients.Clients, c *v1.Cluster, snapshotName string, expectedConfigMap corev1.ConfigMap, expectedNodeCount int, restoreRKEConfig string) {
	...
	err = wait.PollUntilContextTimeout(clients.Ctx, 30*time.Second, 25*time.Minute, true, func(ctx context.Context) (bool, error) {
		cp, err := clients.RKE.RKEControlPlane().Get(c.Namespace, c.Name, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
...
// RunSnapshotCreatesTest creates multiple etcd snapshots and returns all of them.
// This is useful for tests that need to perform multiple restores using different snapshots.
func RunSnapshotCreatesTest(t *testing.T, clients *clients.Clients, c *v1.Cluster, configMap corev1.ConfigMap, targetNode string, count int) []*rkev1.ETCDSnapshot {
	t.Helper()
