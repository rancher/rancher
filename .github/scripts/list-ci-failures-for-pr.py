...
def get_job_logs(repo_owner, repo_name, job_id, headers):
    logs_url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/actions/jobs/{job_id}/logs"
    logs_response = make_github_request(logs_url, headers)
...
def process_job(repo_owner, repo_name, job, run_data, attempt_number, headers):
    if job["conclusion"] != "failure":
        return None

    job_logs = get_job_logs(repo_owner, repo_name, job["id"], headers)
...
def get_jobs_for_attempt(repo_owner, repo_name, run_data, attempt_number, headers):
    ...
    jobs_response = make_github_request(jobs_url, headers)
...
def process_single_attempt(repo_owner, repo_name, run, attempt_number, headers):
    if attempt_number != run.get("run_attempt", 1):
        ...
        attempt_response = make_github_request(attempt_url, headers)
    ...
    jobs_data = get_jobs_for_attempt(repo_owner, repo_name, run_data, attempt_number, headers)
    ...
    with ThreadPoolExecutor(max_workers=min(10, len(jobs_data["jobs"]))) as executor:
        futures = {
            executor.submit(process_job, repo_owner, repo_name, job, run_data, attempt_number, headers): job
...
def get_failed_workflow_attempts(repo_owner, repo_name, pr_number, headers):
    pr_url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/pulls/{pr_number}"
    pr_response = make_github_request(pr_url, headers)
    ...
    runs_response = make_github_request(runs_url, headers)
    ...
    with ThreadPoolExecutor(max_workers=min(10, len(all_attempts))) as executor:
        futures = {
            executor.submit(process_single_attempt, repo_owner, repo_name, run, attempt_number, headers):
...
def main():
    ...
    failed_attempts = get_failed_workflow_attempts(repo_owner, repo_name, pr_number, headers)
