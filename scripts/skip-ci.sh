...
if [ "${DRONE_BUILD_EVENT}" = "push" ] || [ "${DRONE_BUILD_EVENT}" = "pull_request" ]; then
    ...
    if [ $(git diff HEAD~1 --name-only | wc -l) -eq 1 ]; then
        if [ $(git diff HEAD~1 --name-only) = "package/Dockerfile" ]; then
        ...
        if [ $(git diff HEAD~1 --name-only) = "README.md" ] || [ $(git diff HEAD~1 --name-only) = "README-template.md" ]; then
          echo "Only README.md/README-template.md changes found"
          echo "Skipping CI because it is only README change"
          exit 0
        fi

    fi
fi
echo "Not skipping CI"
exit 1
