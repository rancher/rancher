...
func ListenAndServe(ctx context.Context, restConfig *rest.Config, handler http.Handler, bindHost string, httpsPort, httpPort int, acmeDomains []string, noCACerts bool) error {
	...
	// (short and FQDN forms) for the rancher Service as default SANs,
	// alongside the dynamic pod/cluster IPs below. These are
	...
	// present regardless of what the pod-IP filter would otherwise
	// decide -- and unlike those IPs, they don't change across
	// restarts. commonName ("rancher") is the Service backing
	// getClusterIP on this branch -- the split into a separate
	// rancher-internal Service happened in a later release.
	svcName := commonName + "." + namespace.System + ".svc"
	hostIPs = append(hostIPs, "localhost", "127.0.0.1", svcName, svcName+".cluster.local")
