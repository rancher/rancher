...
type CloudCredentialController interface {
	generic.ControllerInterface[*v1.CloudCredential, *v1.CloudCredentialList]
}

// CloudCredentialClient interface for managing CloudCredential resources in Kubernetes.
type CloudCredentialClient interface {
	generic.ClientInterface[*v1.CloudCredential, *v1.CloudCredentialList]
}

// CloudCredentialCache interface for retrieving CloudCredential resources in memory.
type CloudCredentialCache interface {
	generic.CacheInterface[*v1.CloudCredential]
}

// CloudCredentialStatusHandler is executed for every added or modified CloudCredential. Should return the new status to be updated
...
// CloudCredentialGeneratingHandler is the top-level handler that is executed for every CloudCredential event. It extends CloudCredentialStatusHandler by a returning a slice of child objects to be passed to apply.Apply
...
// RegisterCloudCredentialStatusHandler configures a CloudCredentialController to execute a CloudCredentialStatusHandler for every events observed.
...
// RegisterCloudCredentialGeneratingHandler configures a CloudCredentialController to execute a CloudCredentialGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
...
// Handle executes the configured CloudCredentialGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *cloudCredentialGeneratingHandler) isNewResourceVersion(obj *v1.CloudCredential) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *cloudCredentialGeneratingHandler) storeResourceVersion(obj *v1.CloudCredential) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
