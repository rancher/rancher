...
func (c *Client) CronJobs(namespace string) CronJobInterface {
	...
	if err != nil {
		logrus.Errorf("Failed to add user agent to [CronJobs] client: %v", err)
		client = sharedClient
	}
	objectClient := objectclient.NewObjectClient(namespace, client, &CronJobResource, CronJobGroupVersionKind, cronJobFactory{})
