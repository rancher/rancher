...
@test "get version" {
    ...
    run ./kontainer-engine get-version my-super-cluster-name-1

    assert_output --partial "my-super-cluster-name-1: 1.8.2"
}

@test "set version" {
    # make this set an old version
    ./kontainer-engine create --base-url http://localhost:8500 --driver aks --resource-group kube --public-key ./integration-tests/test-key.pub --client-id 12345 --client-secret 67890 --subscription-id 1029384857 my-super-cluster-name-1 > /dev/null 2>&1
    # set new version
    ./kontainer-engine set-version --version 1.8.2 my-super-cluster-name-1  > /dev/null 2>&1
