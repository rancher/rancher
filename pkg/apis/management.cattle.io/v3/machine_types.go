...
type NodeDriverStatus struct {
	// Conditions is a representation of the current state of the driver, this includes its installation status
	// (Downloaded, Installed), as well as its current state (Active, Inactive).
	// Information related to errors encountered while transitioning to one of these states will be populated in the Message and Reason fields.
	// +optional
	Conditions []Condition `json:"conditions"`
