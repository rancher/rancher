...
type NodeDriverStatus struct {
	// Conditions is a representation of the current of the driver
	// +optional
	Conditions []Condition `json:"conditions"`
	// AppliedURL is the url last used to download the driver
	// +optional
	AppliedURL string `json:"appliedURL"`
	// AppliedChecksum is the last known checksum of the driver, and is used to determine when
	...
	// config object. This field is only specified for drivers bundled within rancher-machine
	// +optional
...
type NodeDriverSpec struct {
	...
	// to be able to create cloud credentials within the Rancher UI
	// +optional
