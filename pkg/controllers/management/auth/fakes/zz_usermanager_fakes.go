...
//
//	mockgen -package auth -source=pkg/user/manager.go
//
...
func (m *MockManager) EnsureClusterToken(clusterName string, input user.TokenInput) (string, error) {
	m.ctrl.T.Helper()
...
func (m *MockManager) EnsureToken(input user.TokenInput) (string, error) {
	m.ctrl.T.Helper()
...
func (m *MockManager) GetKubeconfigToken(clusterName, tokenName, description, kind, userName string, userPrincipal v3.Principal) (*v1.Token, string, error) {
	m.ctrl.T.Helper()
