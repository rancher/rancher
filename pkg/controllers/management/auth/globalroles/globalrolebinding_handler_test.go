...
func TestUpdateA(t *testing.T) {
	...
	grListerMock := fakes.GlobalRoleListerMock{
		GetFunc: func(namespace string, name string) (*v3.GlobalRole, error) {
			return &v3.GlobalRole{}, nil
		},
	}
	grbListerMock := fake.NewMockNonNamespacedCacheInterface[*v3.GlobalRoleBinding](ctrl)
	grbClientMock := fake.NewMockNonNamespacedControllerInterface[*v3.GlobalRoleBinding, *v3.GlobalRoleBindingList](ctrl)
	a := v3.GlobalRoleBinding{
		ObjectMeta: metav1.ObjectMeta{
			Name:        "test-grb",
			UID:         "1234",
			Annotations: map[string]string{"authz.management.cattle.io/crb-name": "cattle-globalrolebinding-test-grb"},
		},
		TypeMeta: metav1.TypeMeta{
			APIVersion: apisv3.GlobalRoleBindingGroupVersionKind.GroupVersion().String(),
			Kind:       apisv3.GlobalRoleBindingGroupVersionKind.Kind,
		...
		UserName:       "user",
		Status: v3.GlobalRoleBindingStatus{
			ObservedGenerationLocal:  0,
			ObservedGenerationRemote: 0,
			LastUpdateTime:           mockTime().String(),
			SummaryLocal:             status.SummaryCompleted,
			LocalConditions: []metav1.Condition{
				{
	...
	grbListerMock.EXPECT().Get(gomock.Any()).Return(&v3.GlobalRoleBinding{
		ObjectMeta: metav1.ObjectMeta{
			Name:        "test-grb",
			UID:         "1234",
			Annotations: map[string]string{"authz.management.cattle.io/crb-name": "cattle-globalrolebinding-test-grb"},
	...
	grbClientMock.EXPECT().UpdateStatus(&a).Return(nil, nil)
	...
	grbLifecycle.status.TimeNow = mockTime
	err := grbLifecycle.updateStatus(&v3.GlobalRoleBinding{ObjectMeta: metav1.ObjectMeta{
		Name: "test-grb",
		UID:  "1234",
	},
		TypeMeta: metav1.TypeMeta{
