...
func (c *Controller) setDeleteNodeAnnotation(node *v3.Node) (*v3.Node, error) {
	...
	if err != nil {
		return node, fmt.Errorf("[nodepool] Failed to update node delete annotation [%s]: %v", node.Name, err)
	}
	return node, nil
}
...
func (c *Controller) deleteNode(node *v3.Node, duration time.Duration) error {
	if deleteAnno, ok := node.Annotations[DeleteNodeAnnotation]; ok && deleteAnno == "true" {
		logrus.Debugf("[nodepool] node already annotated for delete: %s", node.Name)
		return nil
	}
	...
	if duration > time.Duration(0) {
		go func() {
			logrus.Debugf("[nodepool] deleting node %s in %s", newNode.Name, duration.String())
			time.Sleep(duration)
			logrus.Debugf("[nodepool] deleting node %s", newNode.Name)
			err := c.Nodes.DeleteNamespaced(newNode.Namespace, newNode.Name, &metav1.DeleteOptions{
				PropagationPolicy: &f,
			})
			if err != nil && !apierrors.IsNotFound(err) {
				logrus.Errorf("[nodepool] error deleting node %s: %s", newNode.Name, err)
			}
		}()
		return nil
	}
