	...
	for _, node := range nodes {
		...
		if !nodehelper.IsMachineReady(node) {
			...
			if v32.NodeConditionUpgraded.IsUnknown(node) {
				...
				logrus.Debugf("cluster [%s] worker-upgrade: node [%s] is not ready and the node condition Upgraded status is unknown", node.Namespace, node.Name)
			} else {
		...
		if node.Status.AppliedNodeVersion == expectedVersion {
			if v32.NodeConditionUpgraded.IsUnknown(node) {
				...
				logrus.Debugf("cluster [%s] worker-upgrade: node [%s] is upgraded but the node condition Upgraded status is unknown", node.Namespace, node.Name)
			}
			...
			if !node.Spec.InternalNodeSpec.Unschedulable {
				...
				if !v32.NodeConditionUpgraded.IsUnknown(node) && node.Spec.DesiredNodeUnschedulable != "false" {
					...
					logrus.Debugf("cluster [%s] worker-upgrade: node [%s] is upgraded and the node condition Upgraded status is not unknown", node.Namespace, node.Name)
				}
			}
			continue
		}
		...
		if preparedNode(node, drain) {
			...
			status.upgrading++
			status.toProcess = append(status.toProcess, node)
			logrus.Debugf("cluster [%s] worker-upgrade: node [%s] has been cordoned or drained", node.Namespace, node.Name)
			continue
		}

		status.toPrepare = append(status.toPrepare, node)
		logrus.Debugf("cluster [%s] worker-upgrade: node [%s] can be prepared", node.Namespace, node.Name)
	}
