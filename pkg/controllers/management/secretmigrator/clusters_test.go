...
func TestMigrateRKESecrets(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			secretName:        cluster.Spec.ClusterSecrets.RKEBastionHostSSHKeySecret,
			key:               SecretKey,
			expected:          "sshKey",
		},
		{
			...
			cleanupVerifyFunc: emptyStringCondition(
				cluster.Spec.RancherKubernetesEngineConfig.PrivateRegistries[1].ECRCredentialPlugin.AwsSecretAccessKey,
				cluster.Spec.RancherKubernetesEngineConfig.PrivateRegistries[1].ECRCredentialPlugin.AwsSessionToken,
			),
			secretName: cluster.Spec.ClusterSecrets.RKEPrivateRegistryECRSecret,
			key:        "testurl2",
			expected:   `{"awsSecretAccessKey":"secret","awsAccessToken":"token"}`,
		},
		{
			...
			secretName: cluster.Spec.ClusterSecrets.RKEKubeletExtraEnvSecret,
			key:        SecretKey,
			expected:   "secret",
		},
		{
			...
			secretName: cluster.Spec.ClusterSecrets.RKESecretsEncryptionProvidersSecret,
			key:        SecretKey,
