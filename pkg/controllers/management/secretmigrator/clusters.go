...
func (h *handler) sync(_ string, cluster *apimgmtv3.Cluster) (runtime.Object, error) {
	...
	if apimgmtv3.ClusterConditionSecretsMigrated.IsTrue(cluster) &&
		apimgmtv3.ClusterConditionServiceAccountSecretsMigrated.IsTrue(cluster) &&
		apimgmtv3.ClusterConditionRKESecretsMigrated.IsTrue(cluster) {
...
// CreateOrUpdateRKESecretsEncryptionProvidersSecret accepts an optional secret name and an rkeConfig and creates a
...
func (m *Migrator) CreateOrUpdateRKESecretsEncryptionProvidersSecret(secretName string, rkeConfig *rketypes.RancherKubernetesEngineConfig, owner runtime.Object) (*corev1.Secret, error) {
	if rkeConfig == nil ||
	...
	return m.createOrUpdateSecretForCredential(secretName, SecretNamespace, strData, nil, owner, "cluster", "rkesecretsencryptionproviders")
}

// CreateOrUpdateRKEBastionHostSSHKeySecret accepts an optional secret name and an rkeConfig and creates a
// Secret containing the bastion host ssh key.
// If an owner is passed, the owner is set as an owner reference on the Secret.
// If the ssh key is unset, a secret is not created.
func (m *Migrator) CreateOrUpdateRKEBastionHostSSHKeySecret(secretName string, rkeConfig *rketypes.RancherKubernetesEngineConfig, owner runtime.Object) (*corev1.Secret, error) {
	if rkeConfig == nil || rkeConfig.BastionHost.SSHKey == "" {
		return nil, nil
	}
	return m.createOrUpdateSecretForCredential(secretName, SecretNamespace, rkeConfig.BastionHost.SSHKey, nil, owner, "cluster", "rkebastionhostsshkey")
}

func (m *Migrator) CreateOrUpdateRKEPrivateRegistryECRSecret(secretName string, rkeConfig *rketypes.RancherKubernetesEngineConfig, owner runtime.Object) (*corev1.Secret, error) {
	...
	return m.createOrUpdateSecret(secretName, SecretNamespace, data, nil, owner, "cluster", "rkeprivateregistryecr")
}

// CreateOrUpdateRKEKubeletExtraEnvSecret is
// TODO(jhyde): add a better comment
func (m *Migrator) CreateOrUpdateRKEKubeletExtraEnvSecret(secretName string, rkeConfig *rketypes.RancherKubernetesEngineConfig, owner runtime.Object) (*corev1.Secret, error) {
	if rkeConfig == nil || len(rkeConfig.Services.Kubelet.ExtraEnv) == 0 {
		return nil, nil
	}
	...
	return m.createOrUpdateSecretForCredential(secretName, SecretNamespace, awsSecretKey, nil, owner, "cluster", "rkekubeletextraenv")
}
...
func (h *handler) migrateRKESecrets(cluster *apimgmtv3.Cluster) (*apimgmtv3.Cluster, error) {
	...
	obj, doErr := apimgmtv3.ClusterConditionServiceAccountSecretsMigrated.Do(clusterCopy, func() (runtime.Object, error) {
		...
		clusterCopy, err = h.migrateSecret(clusterCopy, "RKESecretsEncryptionProvidersSecret", "rke secrets encryption providers", &clusterCopy.Spec.ClusterSecrets.RKESecretsEncryptionProvidersSecret, h.migrator.CreateOrUpdateRKESecretsEncryptionProvidersSecret, func(spec *apimgmtv3.ClusterSpec) {
			if spec == nil ||
		...
		// rke bastion host
		clusterCopy, err = h.migrateSecret(clusterCopy, "RKEBastionHostSSHKeySecret", "rke bastion host ssh key", &clusterCopy.Spec.ClusterSecrets.RKEBastionHostSSHKeySecret, h.migrator.CreateOrUpdateRKEBastionHostSSHKeySecret, func(spec *apimgmtv3.ClusterSpec) {
			if spec == nil ||
		...
		clusterCopy, err = h.migrateSecret(clusterCopy, "RKEKubeletExtraEnvSecret", "rke kubelet extra env", &clusterCopy.Spec.ClusterSecrets.RKEKubeletExtraEnvSecret, h.migrator.CreateOrUpdateRKEKubeletExtraEnvSecret, func(spec *apimgmtv3.ClusterSpec) {
			if spec == nil ||
				spec.RancherKubernetesEngineConfig == nil {
				return
			}
		...
		clusterCopy, err = h.migrateSecret(clusterCopy, "RKEPrivateRegistryECRSecret", "rke private registry ecr", &clusterCopy.Spec.ClusterSecrets.RKEPrivateRegistryECRSecret, h.migrator.CreateOrUpdateRKEPrivateRegistryECRSecret, func(spec *apimgmtv3.ClusterSpec) {
			if spec == nil ||
				spec.RancherKubernetesEngineConfig == nil {
				return
			}
