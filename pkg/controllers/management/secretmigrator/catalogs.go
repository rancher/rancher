...
func (h *handler) syncCatalog(key string, catalog *v3.Catalog) (runtime.Object, error) {
	...
	if v3.CatalogConditionSecretsMigrated.IsTrue(catalog) {
		logrus.Tracef("[secretmigrator] catalog %s already migrated", catalog.Name)
		return catalog, nil
	}
	obj, err := v3.CatalogConditionSecretsMigrated.Do(catalog, func() (runtime.Object, error) {
		...
		v3.CatalogConditionSecretsMigrated.True(catalog)
