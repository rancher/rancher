...
import (
	...
	apiprojectv3 "github.com/rancher/rancher/pkg/apis/project.cattle.io/v3"
...
func (h *handler) onClusterChange(key string, cluster *apimgmtv3.Cluster) (*apimgmtv3.Cluster, error) {
	...
	var (
		...
		strategy      apimgmtv3.ClusterUpgradeStrategy
	)
	...
	if !isNewer {
		...
		if !needsUpgrade {
			...
			if apimgmtv3.ClusterConditionUpgraded.IsUnknown(cluster) {
				logrus.Infof("[k3s-based-upgrader] finished upgrading cluster [%s]", cluster.Name)
				apimgmtv3.ClusterConditionUpgraded.True(cluster)
				apimgmtv3.ClusterConditionUpgraded.Message(cluster, "")
	...
	if apimgmtv3.ClusterConditionUpgraded.IsTrue(cluster) {
...
func (h *handler) deployK3sBasedUpgradeController(cluster *apimgmtv3.Cluster) error {
	...
	var appname string
	switch {
	case cluster.Status.Driver == apimgmtv3.ClusterDriverK3s:
		appname = "rancher-k3s-upgrader"
	case cluster.Status.Driver == apimgmtv3.ClusterDriverRke2:
	...
	if err != nil {
		...
		if !checkDeployed(app) {
			if !apiprojectv3.AppConditionForceUpgrade.IsUnknown(app) {
				apiprojectv3.AppConditionForceUpgrade.Unknown(app)
			}
...
func (h *handler) nodesNeedUpgrade(cluster *apimgmtv3.Cluster, version string) (bool, error) {
...
func checkDeployed(app *apiprojectv3.App) bool {
	return apiprojectv3.AppConditionDeployed.IsTrue(app) || apiprojectv3.AppConditionInstalled.IsTrue(app)
}
