...
const (
	handlerName                 = "harvester-machine-provision-cleanup"
...
func Register(ctx context.Context, clients *wrangler.Context) {
	...
	clients.Dynamic.OnChange(ctx, handlerName, validGVK, h.onChange)
}
...
func (h *handler) onChange(obj runtime.Object) (runtime.Object, error) {
	...
	if apierrors.IsNotFound(err) {
		logrus.Warnf("[%s] %s: there is no cluster (apiversion=cluster.x-k8s.io). Continue ...", handlerName, key)
		return obj, nil
	}
	if err != nil {
		logrus.Errorf("[%s] %s: error getting cluster (apiversion=cluster.x-k8s.io): %v", handlerName, key, err)
		return obj, err
	}
	...
	if err != nil {
		logrus.Errorf("[%s] %s: error getting cluster (apiversion=provisioning.cattle.io): %v", handlerName, key, err)
		return obj, err
	}

	secret, err := machineprovision.GetCloudCredentialSecret(h.secretCache, "", cluster.Spec.CloudCredentialSecretName)
	if err != nil {
		logrus.Errorf("[%s] %s: error getting cloud credential secret: %v", handlerName, key, err)
		return obj, err
	}

	restConfig, err := clientcmd.RESTConfigFromKubeConfig(secret.Data["harvestercredentialConfig-kubeconfigContent"])
	if err != nil {
		logrus.Errorf("[%s] %s: error getting REST config from cloud credential secret: %v", handlerName, key, err)
		return obj, err
	}
	...
	if err != nil {
		logrus.Errorf("[%s] %s: error getting dynamic client: %v", handlerName, key, err)
		return obj, err
	}
	...
	if apierrors.IsNotFound(err) {
		logrus.Warnf("[%s] %s: there is no VM (gvr=%s, namespace=%s, name=%s). Continue ...",
			handlerName, key, resourceGVR.String(), resourceNamespace, resourceName)
		return obj, nil
	}
	if err != nil {
		logrus.Errorf("[%s] %s: error getting VM (gvr=%s, namespace=%s, name=%s): %v",
			handlerName, key, resourceGVR.String(), resourceNamespace, resourceName, err)
		return obj, err
	}
