...
// look again.
func (h *handler) reconcileActive(op *opv1alpha1.ETCDSnapshotSave, status opv1alpha1.ETCDSnapshotSaveStatus) (opv1alpha1.ETCDSnapshotSaveStatus, error) {
	...
	if previous, canceled := ops.CancelForRequest(&op.Spec.OperationSpec, &status.OperationStatus); canceled {
		logrus.Infof("[etcdsnapshotsave] %s/%s: marking operation as canceled: cancellation requested in phase [%s] step [%s]", op.Namespace, op.Name, previous, status.Step)
	}
	...
	if ops.Collectable(&op.Spec.OperationSpec, &status.OperationStatus) {
		if err := h.etcdsnapshotsaves.Delete(op.Namespace, op.Name, &metav1.DeleteOptions{}); err != nil {
			return status, err
		}
...
func (h *handler) reconcileDeleting(op *opv1alpha1.ETCDSnapshotSave, status opv1alpha1.ETCDSnapshotSaveStatus) (opv1alpha1.ETCDSnapshotSaveStatus, error) {
	...
	if previous, canceled := ops.CancelForDeletion(&status.OperationStatus); canceled {
		logrus.Infof("[etcdsnapshotsave] %s/%s: marking operation as canceled: deleted in phase [%s] step [%s] before terminal handling completed", op.Namespace, op.Name, previous, status.Step)
	}
...
func (h *handler) advance(op *opv1alpha1.ETCDSnapshotSave, status opv1alpha1.ETCDSnapshotSaveStatus) (opv1alpha1.ETCDSnapshotSaveStatus, error) {
...
func (h *handler) reconcileSave(s *scope, status opv1alpha1.ETCDSnapshotSaveStatus) (opv1alpha1.ETCDSnapshotSaveStatus, error) {
	...
	if concurrency < len(secrets) {
		ops.SetWaitingForPlan(&status.OperationStatus, status.Step, results)
...
func (h *handler) handleTerminal(s *scope, status opv1alpha1.ETCDSnapshotSaveStatus, phase terminalPhase) (opv1alpha1.ETCDSnapshotSaveStatus, error) {
	...
	if honorHook {
		...
		logrus.Debugf("[etcdsnapshotsave] %s/%s: %s with no claim on the beacon, leaving it untouched", s.op.Namespace, s.op.Name, status.Phase)
	}
