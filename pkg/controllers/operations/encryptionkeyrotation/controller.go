...
func (h *handler) onChange(op *opv1alpha1.EncryptionKeyRotation, status opv1alpha1.EncryptionKeyRotationStatus) (opv1alpha1.EncryptionKeyRotationStatus, error) {
	...
	if op.Spec.Cancel && !status.Phase.IsTerminal() {
		logrus.Infof("[encryptionkeyrotation] %s/%s: cancellation requested, unwinding from phase %s", op.Namespace, op.Name, status.Phase)
...
func (h *handler) reconcileRotate(s *scope, status opv1alpha1.EncryptionKeyRotationStatus) (opv1alpha1.EncryptionKeyRotationStatus, error) {
	...
	planStatus, err := h.store.AssignPlan(leader, nodePlan, s.beacon, 1, 1)
	if err != nil {
		return status, err
	}
...
func (h *handler) reconcileRestartNode(
	...
	planStatus, err := h.store.AssignPlan(secret, nodePlan, s.beacon, 5, 5)
	if err != nil {
		return status, false, err
	}
...
func (h *handler) handleCanceled(s *scope, status opv1alpha1.EncryptionKeyRotationStatus) (opv1alpha1.EncryptionKeyRotationStatus, error) {
	...
	// An operation which canceled itself has already recorded why, and never mutated the cluster. Only
	// a requested cancellation records who asked and may leave the cluster requiring a restore.
	if s.op.Spec.Cancel {
