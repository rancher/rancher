...
func (c FSCache) SyncWithIndex(index *SafeIndex, fsCacheFiles []string) error {
	...
	for _, file := range fsCacheFiles {
		...
		if err != nil {
			errors.Join(errs, fmt.Errorf("failed to get chart name and version for file [%s]. Error: %w", file, err))
			continue
		}
		...
		if !ok || index.Entries[chartName].Version != chartVersion {
			...
			if err != nil {
				errors.Join(errs, fmt.Errorf("failed to delete cache entry. Error: %w", err))
				continue
			}
		}
	}
