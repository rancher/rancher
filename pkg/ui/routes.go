...
func New(_ v3.PreferenceCache, clusterRegistrationTokenCache v3.ClusterRegistrationTokenCache) http.Handler {
	...
	// Query parameter based routes - check state param in handler
	...
	// Path prefix routes (register both with and without trailing slash)
	...
	// Final fallback handler (replaces NotFoundHandler)
