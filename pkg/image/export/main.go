...
usage () {
    ...
    echo "  [-l|--image-list path] text file with list of images; one image per line."
    echo "  [-i|--images path] tar.gz generated by docker save."
    echo "  [-p|--platform arch] target arch of images to be saved; Requires docker version 20+ or else enable experimental features"
    echo "  [-h|--help] Usage message"
}
...
while IFS= read -r i; do
    [ -z "${i}" ] && continue
    if docker pull $platform "${i}" > /dev/null 2>&1; then
