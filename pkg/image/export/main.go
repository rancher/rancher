...
func checkImage(image string) error {
	...
	if strings.HasPrefix(image, "weaveworks") || strings.HasPrefix(image, "noiro") || strings.HasPrefix(image, "registry:") || strings.EqualFold(image, "busybox") || strings.HasPrefix(image, "rancherlabs") {
		return nil
	}
...
usage () {
    ...
    echo "  [--windows-versions version] Comma separated Windows versions. e.g., \"1809|lstc2022\". (Default \"1809\")"
