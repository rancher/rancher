...
usage () {
    ...
    echo "  [--windows-image-list path] text file with list of images used in Windows. Windows image mirroring is skipped when it is empty"
...
function usage {
    ...
    echo "  [-i|--images path] tar.gz generated by docker save."
    echo "  [-r|--registry registry:port] target private registry:port."
    echo "  [-o|--os-release-id (1809|1903|...)] release id of OS, could detect automatically."
    echo "  [-h|--help] Usage message."
}
...
if (-not $registry)
{
	echo "Must input registry address !"
    usage
    exit 1
}
...
{
	echo "Could not find '$images' tar.gz !"
    usage
	exit 1
}
...
if (-not (Test-Path $image_list))
{
	echo "Could not find '$image_list' text file !"
    usage
	exit 1
}
