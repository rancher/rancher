...
function usage {
    ...
    echo "  [-o|--os-release-id (1809|lstc2022|...)] release id of OS. Only used when specified."
    echo "  [-h|--help] Usage message."
}
...
if ($os_release_id)
{
    $image_suffix = ("-windows-{0}" -f $os_release_id)
}

Get-Content -Force -Path $image_list | ForEach-Object {
    if ($_) {
        $fullname_image = ('{0}{1}' -f $_, $image_suffix)
...
function usage {
    ...
    echo "  [-o|--os-release-id (1809|ltsc2022|...)] release id of OS. Only used when specified."
    echo "  [-h|--help] Usage message."
}
...
Get-Content -Force -Path $image_list | ForEach-Object {
    if ($_) {
        $fullname_image = ('{0}{1}{2}' -f $source_registry, $_, $image_suffix)
