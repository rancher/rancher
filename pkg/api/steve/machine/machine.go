...
func Register(server *steve.Server, clients *wrangler.ProvisioningCtx) {
	sshClient := &sshClient{
		machines: clients.CAPI.Machine(),
		secrets:  clients.Core.Secret(),
	}

	go func(server *steve.Server) {
		for {
			time.Sleep(time.Second * 5)
			if sshClient.machines == nil {
				logrus.Info("Waiting for CAPI CRDs to be available before registering CAPI Machines to Steve")
				continue
			}
			logrus.Infof("Registering CAPI Machines to Steve")
