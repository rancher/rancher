...
func TestLocalCluster(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			config, err := RestConfigForURL(server.URL, defaultToken)
			assert.NoError(t, err, "error when creating rest client")
			sarWrapper, err := typedv1.NewForConfig(config)
			assert.NoError(t, err, "error when creating sar wrapper")

			proxyMiddleware, err := proxy.NewProxyMiddleware(sarWrapper.SubjectAccessReviews(), &defaultDialer{}, &fakeClusterCache{}, true, &localHandler)
...
func (d *defaultDialer) ClusterDialer(clusterID string) func(ctx context.Context, network, address string) (net.Conn, error) {
...
func RestConfigForURL(serverURL string, token string) (*rest.Config, error) {
	config := v1.Config{
