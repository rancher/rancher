...
var rkeNetworkPolicyControllerPlugins = map[string]bool{
	...
	"weave":  true,
	"aci":    true,
	"none":   true, // none is included because user may provide a custom CNI that supports network policies
}
...
func validateNetworkFlag(data map[string]interface{}, create bool) error {
	...
	if enableNetworkPolicy == nil && create {
		...
		if ingressProvider == "nginx" && plugin != "canal" && ingressNetworkMode != "hostPort" && ingressNetworkMode != "none" {
