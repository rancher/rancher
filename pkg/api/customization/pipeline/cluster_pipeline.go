...
func (h *ClusterPipelineHandler) authuser(apiContext *types.APIContext) error {
	...
	if !isSourceCodeTypeConfigured(clusterPipeline, authUserInput.SourceCodeType) {
		return fmt.Errorf("%s is not configured", authUserInput.SourceCodeType)
	}
...
func isSourceCodeTypeConfigured(clusterPipeline *v3.ClusterPipeline, sourceCodeType string) bool {
	if sourceCodeType == model.GithubType && clusterPipeline.Spec.GithubConfig != nil {
		return true
	} else if sourceCodeType == model.GitlabType && clusterPipeline.Spec.GitlabConfig != nil {
		return true
