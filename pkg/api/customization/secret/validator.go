...
func Validator(request *types.APIContext, schema *types.Schema, data map[string]interface{}) error {
	if request.Method == http.MethodPost {
		...
		if err := request.AccessControl.CanDo(v1.SecretGroupVersionKind.Group, v1.SecretResource.Name, "update", request, secretState, schema); err != nil {
			return httperror.NewAPIError(httperror.PermissionDenied, fmt.Sprintf("unable to validate: user does not have permission"))
		}
		...
		if err := access.ByID(request, request.Version, request.Type, request.ID, &secretState); err != nil {
			return httperror.NewAPIError(httperror.NotFound, fmt.Sprintf("unable to validate: %v", err))
		}
		// this is unused but will be necessary if readonly users are ever given permission to view secrets
		if err := request.AccessControl.CanDo(v1.SecretGroupVersionKind.Group, v1.SecretResource.Name, "update", request, secretState, schema); err != nil {
			return httperror.NewAPIError(httperror.PermissionDenied, "unable to validate: user does not have permission")
		}
	}
	return nil
}
