...
func Test_refreshAttributes(t *testing.T) {
	...
	eLoginSecretLocal := corev1.Secret{
		...
		Data: map[string][]byte{
			...
			"is-login":         []byte("true"),
			"ttl":              []byte("4000"),
			"user-id":          []byte("user-abcde"),
			"hash":             []byte("kla9jkdmj"),
			"auth-provider":    []byte(providers.LocalProvider),
			"last-update-time": []byte("13:00:05"),
			"display-name":     []byte(""),
			"login-name":       []byte("admin"),
			"principal-id":     []byte("local://user-abcde"),
	...
	eDerivedSecretShibboleth := corev1.Secret{
		...
		Data: map[string][]byte{
			...
			"user-id":          []byte("user-abcde"),
			"hash":             []byte("kla9jkdmj"),
			"auth-provider":    []byte(saml.ShibbolethName),
			"last-update-time": []byte("13:00:05"),
			"display-name":     []byte(""),
			"login-name":       []byte("user1"),
			"principal-id":     []byte("shibboleth_user://user1"),
			"kube-uid":         []byte("2905498-kafld-lkad"),
		},
	}
	...
	eTokenSetupDerivedLocal := func(
		...
		secrets.EXPECT().
			...
			AnyTimes()
		secrets.EXPECT().
			Get("cattle-tokens", gomock.Any(), gomock.Any()).
	...
	eTokenSetupLocal := func(
		...
		secrets.EXPECT().
			...
			AnyTimes()
		secrets.EXPECT().
			Get("cattle-tokens", "user-abcde-login-local", gomock.Any()).
			Return(&eLoginSecretLocal, nil).
			AnyTimes()
		secrets.EXPECT().
			Get("cattle-tokens", "user-abcde-derived-local", gomock.Any()).
			Return(&eDerivedSecretLocal, nil).
			AnyTimes()
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			r := &refresher{
				...
				extTokenStore: exttokens.NewSystem(nil, secrets, uattrs, users,
					exttokens.NewTimeHandler(),
					exttokens.NewHashHandler(),
					exttokens.NewPermissionHandler()),
			}
