...
func TestDefaultPolicies(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			Headers:  http.Header{"Content-Type": {contentTypeJSON}},
			Body:     []byte(`{"metadata":{"namespace":"default","name":"my_configmap"},"data":{"foo":"c3VwZXIgc2VjcmV0IGRhdGE=\\", "bar": "U3VwZXIgU2VjcmV0IERhdGEK"}, "accessToken" : "fake_access_token"}`),
			Expected: []byte(fmt.Sprintf(`{"metadata":{"namespace":"default","name":"my_configmap"},"data":{"foo":"c3VwZXIgc2VjcmV0IGRhdGE=\\", "bar": "U3VwZXIgU2VjcmV0IERhdGEK"}, "accessToken" : "%s"}`, redacted)),
		},
		{
			...
			Expected: []byte(fmt.Sprintf(`{"type": "cluster", "rancherKubernetesEngineConfig": {"services": {"kubeApi": {"secretsEncryptionConfig": "%s"}}}}`, redacted)),
		},
		{
			...
			Expected: []byte(fmt.Sprintf(`{"normalField": "some data", "manifestUrl": "%s", "insecureWindowsNodeCommand": "%[1]s", "insecureNodeCommand": "%[1]s", "insecureCommand": "%[1]s", "command": "%[1]s", "windowsNodeCommand": "%[1]s"}`, redacted)),
		},
	}
