...
func (ap *azureProvider) testAndApply(actionName string, action *types.Action, request *types.APIContext) error {
	...
	// This covers the case where users upgrade Rancher to v2.5.16+ without having used Azure AD as the auth provider.
	// In 2.5.16+, whether Azure AD is later registered or not, Rancher on startup creates the annotation on the template auth config.
	// But in the case where the auth config had been created on Rancher startup prior to v2.5.16, the annotation would be missing.
