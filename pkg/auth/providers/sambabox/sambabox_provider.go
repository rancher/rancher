...
func (p *adProvider) TransformToAuthProvider(authConfig map[string]interface{}) (map[string]interface{}, error) {
	...
	defaultDomain := ""
	if dld, ok := authConfig[client.SambaBoxProviderFieldDefaultLoginDomain].(string); ok {
		defaultDomain = dld
	}
	ap[client.SambaBoxProviderFieldDefaultLoginDomain] = defaultDomain
	return ap, nil
}
...
func (p *adProvider) getSambaBoxConfig() (*v32.SambaBoxConfig, *x509.CertPool, error) {
	// TODO See if this can be simplified. also, this makes an api call everytime. find a better way
	authConfigObj, err := p.authConfigs.ObjectClient().UnstructuredClient().Get("sambabox", metav1.GetOptions{})
	if err != nil {
		return nil, nil, fmt.Errorf("failed to retrieve SambaBoxConfig, error: %v", err)
	}

	u, ok := authConfigObj.(runtime.Unstructured)
	if !ok {
		return nil, nil, fmt.Errorf("failed to retrieve SambaBoxConfig, cannot read k8s Unstructured data")
	}
	storedADConfigMap := u.UnstructuredContent()

	storedADConfig := &v32.SambaBoxConfig{}
	mapstructure.Decode(storedADConfigMap, storedADConfig)

	metadataMap, ok := storedADConfigMap["metadata"].(map[string]interface{})
	if !ok {
		return nil, nil, fmt.Errorf("failed to retrieve SambaBoxConfig metadata, cannot read k8s Unstructured data")
	}
	...
	if storedADConfig.ServiceAccountPassword != "" {
		value, err := common.ReadFromSecret(p.secrets, storedADConfig.ServiceAccountPassword,
			strings.ToLower(v3client.SambaBoxConfigFieldServiceAccountPassword))
		if err != nil {
			return nil, nil, err
		}
...
func (p *adProvider) CanAccessWithGroupProviders(userPrincipalID string, groupPrincipals []v3.Principal) (bool, error) {
	config, _, err := p.getSambaBoxConfig()
	if err != nil {
