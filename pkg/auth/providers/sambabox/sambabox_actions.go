...
func (p *adProvider) testAndApply(actionName string, action *types.Action, request *types.APIContext) error {
	input, err := handler.ParseAndValidateActionBody(request, request.Schemas.Schema(&managementschema.Version,
		client.SambaBoxTestAndApplyInputType))
	if err != nil {
		return err
	}
	configApplyInput := &v32.SambaBoxTestAndApplyInput{}
	if err := mapstructure.Decode(input, configApplyInput); err != nil {
		return httperror.NewAPIError(httperror.InvalidBodyContent,
			fmt.Sprintf("Failed to parse body: %v", err))
	}

	config := &configApplyInput.SambaBoxConfig
	...
	if config.ServiceAccountPassword != "" {
		value, err := common.ReadFromSecret(p.secrets, config.ServiceAccountPassword,
			strings.ToLower(client.SambaBoxConfigFieldServiceAccountPassword))
		if err != nil {
			return err
		}
	...
	err = p.saveSambaBoxConfig(config)
	if err != nil {
		return httperror.NewAPIError(httperror.ServerError, fmt.Sprintf("Failed to save sambabox config: %v", err))
	}
...
func (p *adProvider) saveSambaBoxConfig(config *v32.SambaBoxConfig) error {
	storedConfig, _, err := p.getSambaBoxConfig()
	if err != nil {
		return err
	}
	config.APIVersion = "management.cattle.io/v3"
	config.Kind = v3.AuthConfigGroupVersionKind.Kind
	config.Type = client.SambaBoxConfigType
	config.ObjectMeta = storedConfig.ObjectMeta

	field := strings.ToLower(client.SambaBoxConfigFieldServiceAccountPassword)
	...
	logrus.Debugf("updating sambaBoxConfig")
