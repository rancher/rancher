...
func PerformSamlLogout(apiContext *types.APIContext, token accessor.TokenAccessor) error {
...
func PerformSamlLogoutAll(apiContext *types.APIContext, token accessor.TokenAccessor) error {
	providerName := token.GetAuthProvider()
...
func (s *Provider) toPrincipal(principalType string, princ v3.Principal, token accessor.TokenAccessor) v3.Principal {
	if principalType == s.userType {
		...
		if token != nil {
			princ.Me = s.isThisUserMe(token.GetUserPrincipal(), princ)
			if princ.Me {
