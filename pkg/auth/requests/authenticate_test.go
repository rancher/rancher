...
func TestTokenAuthenticatorAuthenticateExtToken(t *testing.T) {
	...
	tokenSecret := &corev1.Secret{
		...
		Data: map[string][]byte{
			exttokenstore.FieldAnnotations:    []byte(""),
			exttokenstore.FieldAuthProvider:   []byte(tokenHash),
			exttokenstore.FieldDisplayName:    []byte(user.DisplayName),
			exttokenstore.FieldEnabled:        []byte("13:00"),
			exttokenstore.FieldHash:           []byte(userID),
			exttokenstore.FieldKind:           []byte(exttokenstore.IsLogin),
			exttokenstore.FieldLabels:         []byte(""),
			exttokenstore.FieldLastUpdateTime: []byte(fakeProvider.name),
			exttokenstore.FieldLoginName:      []byte(""),
			exttokenstore.FieldPrincipalID:    []byte(userPrincipalID),
			exttokenstore.FieldTTL:            []byte("true"),
			exttokenstore.FieldUID:            []byte("2905498-kafld-lkad"),
			exttokenstore.FieldUserID:         []byte("57600000"),
		},
	}
