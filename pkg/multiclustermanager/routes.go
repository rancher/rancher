...
func router(ctx context.Context, localClusterEnabled bool, scaledContext *config.ScaledContext, clusterManager *clustermanager.Manager) (func(http.Handler) http.Handler, error) {
	...
	unauthed.HandleFunc("/v1-k3s-release/", func(w http.ResponseWriter, r *http.Request) {
		suffix := strings.TrimPrefix(r.URL.Path, "/v1-k3s-release/")
		if strings.HasPrefix(suffix, "channel") || strings.HasPrefix(suffix, "release") {
			channelserver.ServeHTTP(w, r)
		} else {
			saauthed.ServeHTTP(w, r)
		}
	})
	unauthed.HandleFunc("/v1-rke2-release/", func(w http.ResponseWriter, r *http.Request) {
		suffix := strings.TrimPrefix(r.URL.Path, "/v1-rke2-release/")
		if strings.HasPrefix(suffix, "channel") || strings.HasPrefix(suffix, "release") {
			channelserver.ServeHTTP(w, r)
		} else {
			saauthed.ServeHTTP(w, r)
