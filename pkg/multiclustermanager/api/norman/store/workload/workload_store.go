...
import (
	...
	"github.com/rancher/rancher/pkg/multiclustermanager/api/norman/customization/workload"
...
func getCreds(apiContext *types.APIContext, namespaceID string) map[string][]corev1.LocalObjectReference {
	...
	if err := access.List(apiContext, &schema.Version, "namespacedDockerCredential", &types.QueryOptions{}, &namespacedCreds); err == nil {
	...
	if err := access.List(apiContext, &schema.Version, "dockerCredential", &types.QueryOptions{}, &creds); err == nil {
...
func imageUpdated(apiContext *types.APIContext, data map[string]interface{}) (bool, error) {
	...
	accessError := access.ByID(apiContext, &schema.Version, "workload", apiContext.ID, &workload)
	if accessError != nil {
