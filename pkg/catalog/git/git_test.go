...
func TestClone(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			assert.NoError(t, resetEnvVars(oldVars))
		})
	}
}
...
func setMockGitExecutable(t *testing.T) error {
	assert.NoError(t, os.Setenv("TEST_PATH", testPath))
	...
	tempDir := t.TempDir()
	path := os.Getenv("PATH")
	assert.NoError(t, os.Setenv("PATH", tempDir+string(os.PathListSeparator)+path))
...
func setTestOutput(t *testing.T, output string) {
	assert.NoError(t, os.Setenv("TEST_OUTPUT", output))
}

func setTestExitCode(t *testing.T, code int) {
	assert.NoError(t, os.Setenv("TEST_EXIT_CODE", strconv.Itoa(code)))
}

var envVars = []string{"TEST_PATH", "TEST_OUTPUT", "TEST_EXIT_CODE", "PATH"}

func getCurrentEnvVars() map[string]string {
	retMap := map[string]string{}
	for i := range envVars {
		retMap[envVars[i]] = os.Getenv(envVars[i])
	}
	return retMap
}

func resetEnvVars(oldValues map[string]string) error {
