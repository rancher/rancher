...
func (p *Planner) reconcile(controlPlane *rkev1.RKEControlPlane, tokensSecret plan.Secret, clusterPlan *plan.Plan, required bool, tierName string,
	...
	for _, entry := range entries {
		...
		plan, joinedURL, err := p.desiredPlan(controlPlane, tokensSecret, entry, joinURL, resetFailureCountOnSystemAgentRestart)
		if err != nil {
			return err
		}
...
func (p *Planner) desiredPlan(controlPlane *rkev1.RKEControlPlane, tokensSecret plan.Secret, entry *planEntry, joinServer string, resetFailureCountOnSystemAgentRestart bool) (plan.NodePlan, string, error) {
	...
	nodePlan, err = p.addInstallInstructionWithRestartStamp(nodePlan, controlPlane, entry, resetFailureCountOnSystemAgentRestart)
	if err != nil {
		return nodePlan, joinedTo, err
	}
