...
func ParseSnapshotClusterSpecOrError(snapshot *rkev1.ETCDSnapshot) (*provv1.ClusterSpec, error) {
	...
	if snapshot.SnapshotFile.Metadata == "" {
		return nil, fmt.Errorf("%s: metadata map is empty; %q missing", metaPrefix, SnapshotMetadataClusterSpecKey)
	}
	...
	raw, ok := md[SnapshotMetadataClusterSpecKey]
	if !ok || strings.TrimSpace(raw) == "" {
		return nil, fmt.Errorf("%s: %q key not found or empty in snapshot metadata", metaPrefix, SnapshotMetadataClusterSpecKey)
	}

	spec, err := DecompressClusterSpec(raw)
	if err != nil {
		return nil, fmt.Errorf("%s: failed to decode %q payload: %w", metaPrefix, SnapshotMetadataClusterSpecKey, err)
	}
...
func RestoreModeRequiresClusterSpec(r *rkev1.ETCDSnapshotRestore) bool {
	...
	if mode == "" || mode == RestoreRKEConfigNone {
		return false
	}
	return mode == RestoreRKEConfigKubernetesVersion || mode == RestoreRKEConfigAll
}
