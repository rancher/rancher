...
func skipIndexFiltering() bool {
	...
	// If the server is version is dev we skip filtering
	// If the version contains 'head' and setting is true, we skip filtering.
	if serverVersion == "dev" ||
		(strings.Contains(serverVersion, "head") && settings.SkipHelmIndexFiltering.Get() == "true") {
		return true
	}
	return false
}
