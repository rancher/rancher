...
func (m *MockpermissionHandler) UserHasPermission(token *v1.Token, ctx ext.Context) bool {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "UserHasPermission", token, ctx)
	ret0, _ := ret[0].(bool)
	return ret0
}

// UserHasPermission indicates an expected call of UserHasPermission.
func (mr *MockpermissionHandlerMockRecorder) UserHasPermission(token, ctx any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "UserHasPermission", reflect.TypeOf((*MockpermissionHandler)(nil).UserHasPermission), token, ctx)
}
