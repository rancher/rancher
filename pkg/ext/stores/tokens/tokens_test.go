...
var (
	...
	notFoundError            = apierrors.NewNotFound(schema.GroupResource{}, "bogus")
...
func Test_Store_Delete(t *testing.T) {
	...
	t.Run("not owned, no permission, not found", func(t *testing.T) {
		...
		checker.EXPECT().UserHasPermission(gomock.Any(), gomock.Any()).
			Return(false)
...
func Test_Store_Watch(t *testing.T) {
	...
	t.Run("receive event for owned secret", func(t *testing.T) {
		...
		assert.True(t, more)
		assert.Equal(t, watch.EventType("update"), event.Event)
