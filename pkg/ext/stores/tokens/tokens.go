...
func (t *Store) Create(ctx extcore.Context, token *ext.Token, opts *metav1.CreateOptions) (*ext.Token, error) {
	if !t.checker.UserHasPermission(token, ctx) {
...
func (t *Store) Update(ctx extcore.Context, token *ext.Token, opts *metav1.UpdateOptions) (*ext.Token, error) {
	if !t.checker.UserHasPermission(token, ctx) {
		return nil, apierrors.NewNotFound(ctx.GroupVersionResource.GroupResource(), token.Name)
	}
	return t.SystemStore.update(false, token, opts)
}
...
func (t *SystemStore) update(fullPermission bool, token *ext.Token, opts *metav1.UpdateOptions) (*ext.Token, error) {
	currentSecret, err := t.secretClient.Get(TokenNamespace, token.Name, metav1.GetOptions{})
	if err != nil {
...
func (t *Store) List(ctx extcore.Context, opts *metav1.ListOptions) (*ext.TokenList, error) {
	return t.SystemStore.list(false, ctx.User.GetName(), opts)
}
...
func (t *SystemStore) list(fullView bool, user string, opts *metav1.ListOptions) (*ext.TokenList, error) {
	...
	for _, secret := range secrets.Items {
		...
		if !fullView && (token.Spec.UserID != user) {
			continue
		}
...
func (t *Store) Watch(ctx extcore.Context, opts *metav1.ListOptions) (<-chan extcore.WatchEvent[*ext.Token], error) {
	...
	go func() {
		...
		watcher, err := t.secretClient.Watch(TokenNamespace, *opts)
		if err != nil {
			close(ch)
			return
		}
		...
		for {
			...
			case event, more := <-watcher.ResultChan():
				...
				// users can only watch their own tokens
				if token.Spec.UserID != ctxUser {
					continue
				}
...
func (t *Store) Delete(ctx extcore.Context, name string, opts *metav1.DeleteOptions) error {
	...
	// acceptable. this simply triggers the non-owned secret code path, preventing deletion.
...
type permissionHandler interface {
	UserHasPermission(token *ext.Token, ctx extcore.Context) bool
}
...
func (tp *tokenChecker) UserHasPermission(token *ext.Token, ctx extcore.Context) bool {
