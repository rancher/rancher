...
// Store is the interface to the token store seen by the extension API and users. Wrapped
// around a SystemStore it performs the necessary checks to ensure that Users have only access
...
// SystemStore is the interface to the token store used internally by other parts of
...
type SystemStore struct {
	...
	checker             permissionHandler // subsystem for permission checking
}

func NewFromWrangler(wranglerContext *wrangler.Context) extcore.Store[*ext.Token, *ext.TokenList] {
	return New(
...
func (t *Store) Create(ctx extcore.Context, token *ext.Token, opts *metav1.CreateOptions) (*ext.Token, error) {
	if _, err := t.checker.UserHasPermission("create", token, ctx); err != nil {
		return nil, err
	}
	// note: without error permission is implied.

	return t.SystemStore.Create(ctx.GroupVersionResource.GroupResource(), token, opts)
}

func (t *SystemStore) Create(group schema.GroupResource, token *ext.Token, opts *metav1.CreateOptions) (*ext.Token, error) {
...
func (t *Store) Update(ctx extcore.Context, token *ext.Token, opts *metav1.UpdateOptions) (*ext.Token, error) {
	...
	if ctx.User.GetName() == token.Spec.UserID {
		...
		hasPermission, err := t.checker.UserHasPermission("update", token, ctx)
		if err != nil {
			return nil, err
		}
		if hasPermission {
	...
	return t.SystemStore.update(permissionLevel, token, opts)
}

func (t *SystemStore) Update(token *ext.Token, opts *metav1.UpdateOptions) (*ext.Token, error) {
	return t.update(fullUpdatePermission, token, opts)
}

func (t *SystemStore) update(permissionLevel int, token *ext.Token, opts *metav1.UpdateOptions) (*ext.Token, error) {
	currentSecret, err := t.secretClient.Get(TokenNamespace, token.Name, metav1.GetOptions{})
	if err != nil {
...
func (t *Store) Get(ctx extcore.Context, name string, opts *metav1.GetOptions) (*ext.Token, error) {
	// have to get token first before we can check permissions on user mismatch

	token, err := t.SystemStore.Get(name, opts)
	if err != nil {
		return nil, err
	}
	if _, err := t.checker.UserHasPermission("get", token, ctx); err != nil {
		return nil, err
	}
...
func (t *SystemStore) Get(name string, opts *metav1.GetOptions) (*ext.Token, error) {
...
func (t *Store) List(ctx extcore.Context, opts *metav1.ListOptions) (*ext.TokenList, error) {
	// cannot use checkForManageToken here. we have lots of tokens to check, with the same admin value.
	hasPermission, err := t.checker.UserHasManageTokenPermissions(ctx)
	if err != nil {
		return nil, apierrors.NewInternalError(fmt.Errorf("failed to check user permissions: %w", err))
	}
	return t.SystemStore.list(hasPermission, ctx.User.GetName(), opts)
}

func (t *SystemStore) List(opts *metav1.ListOptions) (*ext.TokenList, error) {
	return t.list(true, "", opts)
}

func (t *SystemStore) list(hasPermission bool, user string, opts *metav1.ListOptions) (*ext.TokenList, error) {
...
func (t *Store) Watch(ctx extcore.Context, opts *metav1.ListOptions) (<-chan extcore.WatchEvent[*ext.Token], error) {
	// cannot use checkForManageToken here. we have lots of tokens to check, with the same admin value.
	hasPermission, err := t.checker.UserHasManageTokenPermissions(ctx)
	if err != nil {
		return nil, apierrors.NewInternalError(fmt.Errorf("failed to check user permissions: %w", err))
...
func (t *Store) Delete(ctx extcore.Context, name string, opts *metav1.DeleteOptions) error {
	...
	if _, err := t.checker.UserHasPermission("delete", token, ctx); err != nil {
		return err
	}

	return t.SystemStore.Delete(name, opts)
}

func (t *SystemStore) Delete(name string, opts *metav1.DeleteOptions) error {
...
// permissionHandler is an interface hiding the details of permission checking
...
// tokenChecker is an implementation of the permissionHandler interface.
...
func (tp *tokenChecker) UserHasPermission(verb string, token *ext.Token, ctx extcore.Context) (bool, error) {
...
func (tp *tokenChecker) UserHasManageTokenPermissions(ctx extcore.Context) (bool, error) {
