...
// principal id for the authenticated token from the request context. It has
// access to the store as a means of fetching detailed token data.
func (tp *tokenAuth) ProviderAndPrincipal(ctx context.Context, store *SystemStore) (string, string, error) {
	...
	// phase I: get token id out of the context
	...
	// tokenID := tokenIDs[0]
	tokenID := "<<placeholder for compilation>>"

	// phase II: fetch token - two attempts, as ext and as norman token - ext first
