...
var (
	...
	KDMBranch                           = NewSetting("kdm-branch", "release-v2.15")
	...
	SystemAgentInstallScript            = NewSetting("system-agent-install-script", "https://github.com/rancher/system-agent/releases/download/v0.15.1/install.sh") // To ensure consistency between SystemAgentInstallScript default value and CATTLE_SYSTEM_AGENT_INSTALL_SCRIPT to utilize the local system-agent-install.sh script when both values are equal.
	WinsAgentInstallScript              = NewSetting("wins-agent-install-script", "https://raw.githubusercontent.com/rancher/wins/v0.15.1/install.ps1")
	...
	// TLSInternalCNAllowedServices lists additional Kubernetes Services
	// whose ClusterIP, ExternalIPs, and LoadBalancer ingress IPs are always
	// allowed as CNs on the tls-rancher-internal cert (the :444 listener),
	// independent of the app=rancher pod-IP filter. Comma-separated; each
	// entry is either a bare Service name (assumed to live in
	// cattle-system, since that's where Rancher's own pods run) or a
	// "namespace/name" pair for a Service that lives in a different
	// namespace, e.g. "my-vip-service,other-ns/other-vip-service".
	...
	// possibly managed by infrastructure outside cattle-system) -- which
	// the pod-IP filter added for stale-CN pruning would otherwise reject
	// and continually prune from the cert. Rancher reads this setting once
	// at startup when the tls-rancher-internal listener is configured;
	// changing it requires a Rancher restart to take effect, though
	// changes to the listed Services' IPs themselves are picked up live.
	...
	ChartDefaultBranch                  = NewSetting("chart-default-branch", "release-v2.15")
	...
	MachineProvisionImage               = NewSetting("machine-provision-image", "rancher/machine:v0.15.0-rancher145.1")
